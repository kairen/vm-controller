...
func NewController(
	...
	klog.Info("Setting up the Foo event handlers")
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	klog.Info("Waiting for Foo and Deployment informer caches to sync")
	if ok := cache.WaitForCacheSync(stopCh, c.deploymentsSynced, c.foosSynced); !ok {
		return fmt.Errorf("failed to wait for caches to sync")
	}

	klog.Info("Starting Foo workers")
	// Launch two workers to process Foo resources
	for i := 0; i < threadiness; i++ {
		go wait.Until(c.runWorker, time.Second, stopCh)
	}

	klog.Info("Started Foo workers")
	<-stopCh
	return nil
}
...
func (c *Controller) processNextWorkItem() bool {
	...
	err := func(obj interface{}) error {
		...
		if key, ok = obj.(string); !ok {
			...
			utilruntime.HandleError(fmt.Errorf("Foo controller expected string in workqueue but got %#v", obj))
			return nil
		}
		...
		if err := c.syncHandler(key); err != nil {
			...
			return fmt.Errorf("Foo controller error syncing '%s': %s, requeuing", key, err.Error())
		}
		// Finally, if no error occurs we Forget this item so it does not
		// get queued again until another change happens.
		c.workqueue.Forget(obj)
		klog.Infof("Foo controller successfully synced '%s'", key)
		return nil
	}(obj)
